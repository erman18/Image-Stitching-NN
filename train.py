...
def print_status_bar(iteration, total, loss=None, metrics=None):
    metrics = " - ".join(["{}: {:.4f}".format(m.name, m.result()) for m in (loss or []) + (metrics or [])])
...
def train(height, width, nb_epochs=10, batch_size=32, save_arch=False, load_weights=True):
    ...
    callback_list.append(callbacks.ModelCheckpoint(stitch_model.weight_path, 
    ...
    callback_list.append(callbacks.ModelCheckpoint(stitch_model.weight_path.replace(".h5", "_is.h5"), 
                                            monitor='val_ISMetric', save_best_only=True,
                                            mode='max', save_weights_only=True, 
                                            save_freq='epoch', verbose=2))

    callback_list.append(callbacks.ModelCheckpoint(stitch_model.weight_path.replace(".h5", "_lpips.h5"), 
                                            monitor='val_LPIPSMetric', save_best_only=True,
                                            mode='min', save_weights_only=True, 
                                            save_freq='epoch', verbose=2))
    ...
    metrics = [ PNSRMetric(name="val_PSNRMetric"), 
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
            ...
            print_status_bar(step, steps_per_epoch, [total_loss_avg, style_loss_avg, content_loss_avg, gradient_loss_avg, simple_loss_avg, tv_loss_avg])
            # break
        ...
        for metric in [total_loss_avg, content_loss_avg, style_loss_avg, gradient_loss_avg, simple_loss_avg, tv_loss_avg] + metrics:
            metric.reset_states()
...
def un_train(height, width, nb_epochs=10, batch_size=32, save_arch=False, load_weights=True):
    ...
    val_dataset = un_read_img_dataset(test_indexes, config_data, callee="un_validation_generator", **params)
    ...
    def test_step(batch, return_dict=True):
        ...
        for metric in metrics:
            ...
            # random.shuffle(r)
            for i in r:
                mask = tf.where(y_batch[:, :, :, i*3:(i+1)*3] != 0., 1., 0.)
                if metric.name == "val_ISMetric":
    ...
    def train_step(sample_data, batch_id, batch_size, loss_dict, return_dict=True):
        ...
        with tf.GradientTape() as tape:
            ...
            random.shuffle(r)
            for i in r:
                tl, cl, sl, tv = compute_loss(target_batch=target_batch[:, :, :, i*3:(i+1)*3], output_batch=output_batch)
    ...
    steps_per_epoch = (samples_per_epoch // batch_size)
    validation_steps = (val_count // batch_size)
    cfg.PRINT_INFO(f"validation_steps: {validation_steps}, val_count: {val_count}, batch_size: {batch_size}")
    total_time = time.time()
    ...
    for epoch in range(1, epochs + 1):
        ...
        for (x_batch_train, y_batch_train) in train_dataset:
            ...
            if batchid >= batch_size:
                ...
                print_status_bar(batch_step, steps_per_epoch, [total_loss_avg, style_loss_avg, content_loss_avg, tv_loss_avg])
                batchid = 0
                loss_dict = {"total_loss": 0, "st_loss": 0, "ct_loss": 0, "tv_loss": 0}
                batch_step += 1
            # break
